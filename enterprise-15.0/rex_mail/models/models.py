    ...
    def _send(self, auto_commit=False, raise_exception=False, smtp_session=None):
        ...
        for mail_id in self.ids:
            ...
            try:
                ...
                catchall = bool(ICP.get_param("mail.catchall.enable", False))
                bounce_alias = ICP.get_param("mail.bounce.alias")
                bounce_alias_static = tools.str2bool(
                    ICP.get_param("mail.bounce.alias.static", "False"))
                catchall_domain = ICP.get_param("mail.catchall.domain")
                if catchall and bounce_alias and catchall_domain:
                ...
                if notifs:
                    ...
                    notifs.flush(
                        fnames=['notification_status', 'failure_type', 'failure_reason'], records=notifs)
...
class IrMailServer(models.Model):
    ...
    def send_email(self, message, mail_server_id=None, smtp_server=None, smtp_port=None,
        ...
        restrict = bool(self.env['ir.config_parameter'].sudo(
        ).get_param("mail.restrict.username", False))
        if not restrict:
            _logger.debug(f"没有启用发件人校正，使用原生逻辑发送...")
        ...
        _logger.debug(f"启用发件人校正，发件服务器: {mail_server.name} 发件人：{smtp_user}")
        # Replace Header
        message.replace_header('From', '%s <%s>' %
